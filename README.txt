...
1. **Phase 1 (1_urls.py): Target Acquisition (Selenium + BeautifulSoup)**
...
* `1_urls.py` - Generates `raw_urls.csv`.
* `2_games.py` - Scrapy spider utilizing custom Selenium Middleware; generates `metadata.csv`.
* `3_request.py` - Synchronous scraper for Wikipedia; generates `awards.csv`.
* `4_data_merging.py` - Pandas integration script; generates the final `final_dataset.csv`.
* `report.ipynb` - Jupyter notebook report of the work.
* `data\` - folder containing all the scraped data. 
